...
import { 
  Sparkles, ShieldCheck, Heart, Flame, Refrigerator, 
  Tv, Compass, Shirt, CupSoda, Wind, PawPrint, Sofa, Grid, Plus, Minus, Phone
} from 'lucide-react';
...
const renderExtraIcon = (iconName: string) => {
  ...
  switch (iconName) {
    ...
    case 'Sparkles': return <Sofa {...props} />;
...
export default function Calculator() {
  ...
  const calculateTotal = () => {
    ...
    if (rooms > 1 && service !== 'windows' && service !== 'office') {
      const perRoomCharge = service === 'standard' ? 400 : service === 'deep' ? 850 : 1200;
    ...
    if (bathrooms > 1 && service !== 'windows' && service !== 'office') {
      const perBathCharge = service === 'standard' ? 700 : service === 'deep' ? 1200 : 1500;
      bathSurcharge = (bathrooms - 1) * perBathCharge;
    }

    // Extras sum
    const extrasSum = extras.reduce((sum, item) => sum + (item.price * item.quantity), 0);

    return Math.round(baseComp + areaComp + roomSurcharge + bathSurcharge + extrasSum);
  };
  ...
  const estimateHours = () => {
    ...
    const extrasAdd = extras.reduce((sum, item) => sum + (item.quantity * 0.4), 0);
  ...
  return (
    <section className="relative bg-transparent py-16 sm:py-24 z-10" id="calculator-section">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 animate-fade-in">
          ...
          <p className="mt-4 text-base sm:text-lg text-slate-600 font-sans leading-relaxed">
            Передвигайте ползунки, выбирайте дополнительные опции. Наша цена прозрачна и окончательна. Без скрытых наценок.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 items-start" id="calc_interactive_grid">
          ...
          <div className="lg:col-span-7 space-y-6 sm:space-y-8 glass-panel-white p-6 sm:p-8 rounded-3xl border border-white/50 shadow-xl text-left">
            ...
            <div className="space-y-3">
              <label className="block font-sans text-xs font-extrabold text-slate-500 uppercase tracking-wider">Шаг 1: Выберите тип уборки</label>
              <div className="grid grid-cols-2 gap-2 sm:gap-3 sm:grid-cols-3 lg:grid-cols-5 animate-fade-in">
                {(['standard', 'deep', 'renovation', 'windows', 'office'] as ServiceType[]).map((type) => {
                  ...
                  return (
                    ...
                    >
                      <span className="hyphens-auto break-words max-w-full text-center px-0.5">{shortLabels[type]}</span>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="p-3 bg-blue-50/25 rounded-xl border border-blue-600/5 mt-2">
                <p className="text-xs text-slate-600 leading-relaxed font-sans mt-0.5">
                  <strong className="text-blue-900 font-bold">{BASE_PRICES[service].label}:</strong> {BASE_PRICES[service].desc}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-5 border-t border-slate-100 pt-6">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="font-sans text-xs font-extrabold text-slate-500 uppercase tracking-wider">Шаг 2: Укажите площадь уборки</label>
                ...
                {service !== 'office' && (
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 pt-2">
                    ...
                    {/* Room counts Buttons */}
                    <div className="space-y-2">
                      <label className="block font-sans text-xs font-extrabold text-slate-500 uppercase tracking-wider">Количество комнат</label>
                    ...
                    {/* Bathroom counters */}
                    <div className="space-y-2">
                      <label className="block font-sans text-xs font-extrabold text-slate-500 uppercase tracking-wider">Санузлы (туалет / ванна)</label>
            ...
            <div className="space-y-4 border-t border-slate-100 pt-6">
              <label className="block font-sans text-xs font-extrabold text-slate-500 uppercase tracking-wider">Шаг {service === 'windows' ? '2' : '3'}: Премиум доп-услуги (по желанию)</label>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3 text-left">
                        <span className={`p-2 rounded-xl ${isChosen ? 'bg-blue-100/50' : 'bg-white'}`}>
                          {renderExtraIcon(item.icon)}
                        </span>
                        <div>
                          <h4 className="font-sans text-xs sm:text-sm font-bold text-slate-900 leading-tight">{item.name}</h4>
                          <p className="text-[11px] font-sans text-slate-500 mt-0.5">+{item.price} ₽ / {item.unit}</p>
                        </div>
                      </div>
                      ...
                      {(item.maxQuantity ?? 10) === 1 ? (
                        <button
                          onClick={() => updateExtraQuantity(item.id, isChosen ? -1 : 1)}
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          >
                            {isChosen ? (
                              ...
                              <span className="text-xs font-semibold px-2">Добавить</span>
                            )}
                          </button>
                        </div>
                      )}
            ...
            <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-2xl relative overflow-hidden text-left border border-slate-800">
              ...
              <div className="space-y-4 text-sm pb-5 border-b border-slate-800">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <span className="block font-bold text-blue-400">{BASE_PRICES[service].label}</span>
                    {service !== 'windows' && <span className="text-xs text-slate-400 mt-0.5">{area} м² • {rooms} {rooms === 1 ? 'комната' : rooms < 5 ? 'комнаты' : 'комнат'} • {bathrooms} {bathrooms === 1 ? 'санузел' : 'санузла'}</span>}
                  </div>
                  <span className="font-bold text-slate-100">
                    {BASE_PRICES[service].base + (service !== 'windows' ? Math.max(0, area - 25) * BASE_PRICES[service].perSqm : 0)} ₽
                  </span>
                </div>

                {/* Room extras */}
                {service !== 'windows' && service !== 'office' && (rooms > 1 || bathrooms > 1) && (
                  <div className="flex justify-between text-xs text-slate-400">
                    <span>Доп. комнаты / Сантехника:</span>
                    <span>
                      +{((rooms > 1 ? (rooms - 1) * (service === 'standard' ? 400 : service === 'deep' ? 850 : 1200) : 0) + 
                        (bathrooms > 1 ? (bathrooms - 1) * (service === 'standard' ? 700 : service === 'deep' ? 1200 : 1500) : 0))} ₽
                    </span>
                  </div>
                )}
                ...
                {extras.filter(e => e.quantity > 0).length > 0 && (
                  <div className="space-y-1.5 bg-slate-800/60 p-3 rounded-xl border border-slate-850">
                    <span className="block text-[11px] font-extrabold uppercase text-slate-500 tracking-wider">Выбранный доп-сервис:</span>
                    {extras.filter(e => e.quantity > 0).map(item => (
                      <div key={item.id} className="flex justify-between text-xs text-slate-300">
                        <span>{item.name} {item.quantity > 1 ? `(х${item.quantity})` : ''}</span>
                ...
                <div className="flex justify-between text-xs text-slate-400 pt-1">
                  ...
                  <span className="font-semibold text-slate-200">~ {estimatedHoursCount} ч.</span>
                </div>
              </div>

              {/* Total Visual Box */}
              <div className="py-5 flex items-baseline justify-between">
                <span className="text-sm font-semibold text-slate-450">Финальная стоимость:</span>
              ...
              <div className="bg-slate-800/50 px-4 py-3 rounded-2xl border border-slate-850 space-y-2 mb-6 text-xs text-slate-400 leading-relaxed font-sans">
                ...
                <p>Профессиональный инвентарь, гипоаллергенные эко-средства и выезд в любой район Москвы включены в финальную стоимость!</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span>Позвонить +7 (985) 916-94-82</span>
                </a>
                <p className="text-center text-[11px] text-slate-400 leading-relaxed">
                  Позвоните нам для вызова мастеров по рассчитанной стоимости. Мы зафиксируем цену и подберем удобное для вас время!
                </p>
              </div>
