...
export default function Hero({ onActionClick }: HeroProps) {
  return (
    <section
      className="relative overflow-hidden bg-transparent py-12 sm:py-20 lg:py-24"
      id="hero-section"
    >
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-7 space-y-6 sm:space-y-8 text-left z-10">
            ...
            <p className="max-w-2xl text-base sm:text-lg text-slate-600 font-sans leading-relaxed">
              Регулярная, генеральная и послеремонтная уборка по честным средним
              ценам Москвы. Рассчитайте стоимость за 1 минуту онлайн — платите
              только после полной приемки работы.
            </p>

            {/* Quick Pricing stats */}
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 max-w-lg border-y border-white/40 py-5 sm:py-6">
              <div>
                <dt className="text-xs font-semibold text-slate-400 font-sans uppercase tracking-wider">
                  Цена за м²
                </dt>
                <dd className="mt-1 text-lg font-bold text-slate-900 font-display">
                  от 65 рублей
                </dd>
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-y-2 gap-x-6 pt-2 text-xs text-slate-500 font-sans">
              <span className="flex items-center">
                <ShieldCheck className="mr-1.5 h-4 w-4 text-teal-600" /> Клинеры
                проверены СБ РФ
              </span>
              <span className="flex items-center">
                <Heart className="mr-1.5 h-4 w-4 text-rose-500 fill-rose-500" />{" "}
                Застраховано до 5 млн ₽
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative mt-8 lg:mt-0">
            <div className="relative mx-auto max-w-[440px] lg:max-w-none">
              ...
              <div className="relative overflow-hidden rounded-3xl border border-white/40 shadow-2xl">
                <img
                  src={scandiHeroImg}
                  alt="Премиальная уборка квартир ПрофКлининг"
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-white/70 p-4 rounded-2xl border border-white/70 shadow-lg backdrop-blur-xl flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-100/80 border border-white/60 shadow-xs text-blue-700 font-bold text-lg">
                      5★
                    </span>
                    <div>
                      <h4 className="font-sans text-xs font-extrabold text-slate-950 uppercase tracking-wider text-left">
                        Чистота с гарантией
                      </h4>
                      <p className="text-[11px] font-sans text-slate-500 text-left">
                        Оценка на Яндекс.Услугах 4.9/5
                      </p>
                    </div>
                  </div>
